...
grafana 默认配置文件从容器中拷贝出来：

51d298b61d78：容器Id。

```sh
docker cp 51d298b61d78:/etc/grafana/grafana.ini /opt/grafana/conf
```

注意：注释使用英文分号（`;`）开头，单独占一行。在分号后面的文字，直到该行结尾都全部为注释。

主要修改 `[dataproxy]` 部分超时设置：
...
services:
  ...
  grafana:
    ...
    volumes:
      - /opt/grafana/conf/:/etc/grafana/ # 可以使用默认配置文件，也可以指定配置文件
